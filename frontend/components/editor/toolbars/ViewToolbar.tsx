...
import { ListTree, Maximize, Ruler, Settings2 } from "lucide-react"
...
export default function ViewToolbar({ actions, state, disabled }: { actions?: any; state?: any; disabled?: boolean }) {
    return (
        <div className="flex h-10 items-center gap-1 overflow-x-auto whitespace-nowrap border-b bg-white px-4">
            ...
            <ToolbarIconButton label="Margins" icon={<Settings2 className="h-4 w-4" />} onClick={actions?.onToggleMarginControls} active={state?.showMarginControls} disabled={disabled} />
